...
static void help()
{
  ...
  printk("  --zicfilp-enable-user-cfi     Enable Zicfilp CFI mechanism for user program\n");
...
static void handle_option(const char* arg)
{
  ...
  if (strcmp(arg, "--zicfilp-enable-user-cfi") == 0) {
    zicfilp_enable_user_cfi = true;
    return;
  }
